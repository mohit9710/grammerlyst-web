...
export default function VerbsCarousel() {
  ...
  // Filter the actual fetched verbs
        ...
        <div className="relative max-w-[95%] lg:max-w-7xl mx-auto mb-16">
          {filteredVerbs.length > 0 ? (
            <>
              <button
                onClick={() => scroll("left")}
                className="absolute -left-4 md:-left-8 top-1/2 -translate-y-1/2 z-30 bg-white/80 backdrop-blur-md p-5 rounded-full shadow-xl hover:bg-blue-600 hover:text-white transition-all text-slate-700 border border-slate-100"
              >
              ...
              <div
                ref={sliderRef}
                className="flex gap-8 overflow-x-auto scroll-smooth no-scrollbar snap-x snap-mandatory py-10 px-4"
              >
                {filteredVerbs.map((verb, index) => (
                  <div
                    key={index}
                    onClick={() => setSelectedVerb(verb)}
                    className={`verb-card snap-center cursor-pointer transition-all duration-300 ${
                      selectedVerb?.base === verb.base
                        ? "ring-4 ring-blue-500 ring-offset-8 scale-105"
                        : "hover:scale-102"
                    }`}
                  >
              ...
              <button
                ...
                className="absolute -right-4 md:-right-8 top-1/2 -translate-y-1/2 z-30 bg-white/80 backdrop-blur-md p-5 rounded-full shadow-xl hover:bg-blue-600 hover:text-white transition-all text-slate-700 border border-slate-100"
              >
          ...
          {!selectedVerb ? (
            <div className="text-center p-12 rounded-3xl bg-slate-100/50 border border-slate-200 border-dashed">
              ...
              <p className="text-slate-500 italic text-lg">
                Click a card above to see detailed usage
              </p>
            </div>
          ) : (
