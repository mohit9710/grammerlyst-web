...
const listeningQuestions: ListeningQuestion[] = [
  {
    ...
    transcript:
      "Hello sir, would you like a cappuccino or a latte today?",
    audio:
  ...
  },
  {
    ...
    title: "Job Interview",
    transcript:
      "Can you explain your previous experience in project management?",
    audio:
  ...
  },
  {
    ...
    transcript:
      "Passengers flying to London should proceed to gate number fourteen immediately.",
    audio:
...
const calculateAccuracy = (original: string, typed: string) => {
  const originalWords = cleanText(original).split(/\s+/);
  const typedWords = cleanText(typed).split(/\s+/);
  let matched = 0;
  ...
  typedWords.forEach((word) => {
    if (map.has(word) && (map.get(word) || 0) > 0) {
      matched++;
      map.set(word, (map.get(word) || 0) - 1);
    }
  });
...
export default function ListeningPractice() {
  ...
  const [selectedAccent, setSelectedAccent] = useState<"en-IN" | "en-US" | "en-GB">("en-IN");
  const [micSupported, setMicSupported] = useState(true);

  const recognitionRef = useRef<any>(null);
  const isListeningRef = useRef(false); // track real-time state for keydown handlers
  ...
  useEffect(() => {
    document.title = "Listening Practice | Improve English Listening Skills";
  ...
  // =========================================
  // SPACE HOLD — only when NOT in a textarea/input
  // =========================================
  ...
  useEffect(() => {
    ...
    const handleKeyDown = (e: KeyboardEvent) => {
      ...
      // If user is typing in an input/textarea, let Space work normally (insert space)
      if (isTypingElement()) return;

      // Outside input: prevent page scroll and start speech
      e.preventDefault();

      if (isSpacePressedRef.current) return; // prevent repeated firing on hold
    ...
    const handleKeyUp = (e: KeyboardEvent) => {
      ...
      // If user is typing in an input/textarea, do nothing special
      if (isTypingElement()) return;
  ...
  const startListening = () => {
    ...
    try {
      recognitionRef.current.lang = selectedAccent;
  ...
  const handleCheckAnswer = () => {
    const accuracy = calculateAccuracy(currentQuestion.transcript, userAnswer);
    setScore(accuracy);
    setShowAnswer(true);
    const originalWords = cleanText(currentQuestion.transcript).split(/\s+/);
    const typedWords = cleanText(userAnswer).split(/\s+/);
    const missed = originalWords.filter((word) => !typedWords.includes(word));
          ...
          <div className="flex flex-col lg:flex-row lg:items-center lg:justify-between gap-6">
            <div>
              ...
              <p className="text-slate-500 max-w-2xl">
                Improve your listening skills by hearing real English conversations and typing or
                speaking exactly what you hear.
              </p>
            </div>
            ...
            >
              <option value="en-IN">🇮🇳 Indian Accent</option>
              <option value="en-US">🇺🇸 American Accent</option>
              <option value="en-GB">🇬🇧 British Accent</option>
            </select>
          </div>
        </div>
        ...
        <div className="grid lg:grid-cols-3 gap-8">
          ...
          <div className="lg:col-span-2 bg-white border rounded-[2rem] p-8 shadow-sm">
            <div className="mb-5 flex flex-col md:flex-row md:items-center md:justify-between gap-4">
              <div>
                <h3 className="text-2xl font-black text-slate-900">Type or Speak What You Heard</h3>
            ...
            <div className="flex flex-wrap gap-4 mt-6">
                {isLocked ? (
                    ...
                    className="w-full h-12 bg-gradient-to-r from-yellow-500 to-orange-500 hover:opacity-90 transition-all text-white rounded-2xl font-black shadow-lg flex items-center justify-center"
                    >
                    🔒 Upgrade to Pro
                    </button>
                ) : (
          ...
          <div className="space-y-6">
            ...
            {showAnswer && (
              <div className="bg-white border rounded-[2rem] p-8 shadow-sm">
                <h3 className="text-xl font-black mb-4">Correct Transcript</h3>
                ...
                <div>
                  ...
                  <div className="flex flex-wrap gap-2">
                    {missedWords.length > 0 ? (
                      ...
                      <span className="text-green-600 font-bold">Perfect Listening 🎉</span>
                    )}
                  </div>
                </div>
              </div>
            ...
            <div className="bg-gradient-to-r from-blue-600 to-indigo-600 rounded-[2rem] p-8 text-white shadow-xl">
              <h3 className="text-2xl font-black mb-5">AI Feedback</h3>
              <div className="space-y-4 text-blue-100">
                <div className="flex gap-3">
                  <span>🎧</span>
                  <p>Focus on connected speech and native pronunciation.</p>
                </div>
                <div className="flex gap-3">
                  <span>⚡</span>
                  <p>Replay the audio only 2-3 times for better listening growth.</p>
                </div>
                <div className="flex gap-3">
                  <span>🚀</span>
                  <p>Daily listening improves real-time English comprehension.</p>
                </div>
                <div className="flex gap-3">
                  <span>🎤</span>
                  <p>Speaking your answer aloud helps improve both listening and pronunciation.</p>
                </div>
              </div>
            </div>
          </div>
