...
export default function GrammarWorkshop({
  ...
  const [topics] = useState<GrammarTopic[]>(initialTopics);
  const [activeTopic, setActiveTopic] = useState<GrammarTopic | null>(
  ...
  const handleLessonRead = async (lesson: GrammarLesson) => {
    const token = localStorage.getItem("access_token");

    if (!token) return;
    if (savingLessonId === lesson.id) return;
    ...
    try {
      ...
      await saveAttempt(token, {
        ...
        user_answer: lesson.example_sentence || "",
        corrected_answer: lesson.example_sentence || "",
  ...
  const levels = useMemo(() => {
    const unique = [...new Set(topics.map((t) => t.level))];
    return ["All", ...unique];
  }, [topics]);
  ...
  const filteredTopics = useMemo(() => {
    return topics.filter((topic) => {
      ...
      const matchesLevel =
        selectedLevel === "All" ? true : topic.level === selectedLevel;

      return matchesSearch && matchesLevel;
    });
  }, [topics, search, selectedLevel]);
    ...
    <div className="relative z-10 w-full max-w-[1600px] mx-auto px-4 sm:px-6 lg:px-10 xl:px-16 py-10 lg:py-14">
      ...
      <div className="mb-16 lg:mb-20">
        <div className="inline-flex items-center gap-2 px-4 py-2 rounded-full bg-white/5 border border-white/10 backdrop-blur-xl mb-6">
          ...
          <span className="text-sm text-slate-300">AI Grammar Learning</span>
        </div>
        ...
        <p className="text-slate-400 text-lg max-w-3xl leading-relaxed">
          Learn English grammar with structured lessons, formulas, examples,
          and AI-powered progress tracking.
        </p>
      </div>
      ...
      <div className="grid lg:grid-cols-12 gap-6 xl:gap-10">
        ...
        <aside className="lg:col-span-3">
          <div className="sticky top-24 rounded-[2.5rem] border border-white/10 bg-white/5 backdrop-blur-2xl p-5 xl:p-6 shadow-2xl">
            {/* FILTERS */}
            <div className="mb-8">
              <h2 className="text-2xl font-black mb-5">Grammar Topics</h2>
              ...
              >
                {levels.map((level) => (
                  <option key={level} value={level} className="bg-[#0f172a]">
                    {level}
                  </option>
                ))}
              </select>
                    ...
                    >
                      ...
                      <div className="relative z-10">
                        ...
                        <div className="flex flex-wrap items-center gap-4 mt-6">
                          <button
                            onClick={() => handleLessonRead(lesson)}
